      ...
      <div className="mx-auto max-w-7xl">
        <div className="rounded-3xl bg-[#0d1013] p-6 text-white shadow-[0_20px_55px_rgba(13,16,19,.16)] sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-5">
            ...
            <Link href="/admin/pedidos" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#f4bf32] px-4 text-sm font-black text-black hover:bg-[#e3ae22]">
        ...
        <Card className="mt-6 rounded-3xl border-black/5 p-5 shadow-[0_14px_40px_rgba(17,17,17,.05)] sm:p-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link href="/admin/pedidos" className="text-sm font-black text-[#aa7f18] hover:underline">Ver tudo →</Link>
          </div>
          <div className="mt-5 overflow-hidden rounded-2xl border border-black/5">
            {rows.length === 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody>{rows.slice(0, 5).map((order) => <tr key={order.id} className="border-b border-black/5 hover:bg-black/2"><td className="px-4 py-3 text-sm font-mono text-black/75">{order.id.slice(0, 8)}</td><td className="px-4 py-3"><Badge>{order.status}</Badge></td><td className="px-4 py-3 font-black">R$ {Number(order.total).toFixed(2).replace(".", ",")}</td><td className="px-4 py-3 text-sm text-black/55">{new Date(order.criado_em).toLocaleTimeString("pt-BR")}</td></tr>)}</tbody>
              </table>
            )}
          </div>
        </Card>
