      ...
      <div className="mx-auto max-w-7xl">
        <div className="rounded-3xl bg-[#0d1013] p-6 text-white shadow-[0_20px_55px_rgba(13,16,19,.16)] sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-[-0.04em] sm:text-4xl">Bom-vindo de volta</h1>
              <p className="mt-1 text-sm text-white/55">Você tem {operational.length} pedido{operational.length !== 1 ? "s" : ""} hoje</p>
            </div>
            <Link
              href="/admin/pedidos"
              className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#f4bf32] px-4 text-sm font-black text-black hover:bg-[#e3ae22]"
            >
        ...
        <Card className="mt-6 rounded-3xl border-black/5 p-5 shadow-[0_14px_40px_rgba(17,17,17,.05)] sm:p-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link href="/admin/pedidos" className="text-sm font-black text-[#aa7f18] hover:underline">
              Ver tudo →
            </Link>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-black/5 bg-black/2">
                    <th className="px-4 py-3 text-left text-xs font-black uppercase tracking-[0.12em] text-black/55">ID</th>
                    <th className="px-4 py-3 text-left text-xs font-black uppercase tracking-[0.12em] text-black/55">Status</th>
                    <th className="px-4 py-3 text-left text-xs font-black uppercase tracking-[0.12em] text-black/55">Total</th>
                    <th className="px-4 py-3 text-left text-xs font-black uppercase tracking-[0.12em] text-black/55">Horário</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.slice(0, 5).map((order) => (
                    <tr key={order.id} className="border-b border-black/5 hover:bg-black/2">
                      <td className="px-4 py-3 text-sm font-mono text-black/75">{order.id.slice(0, 8)}</td>
                      <td className="px-4 py-3">
                        <Badge>{order.status}</Badge>
                      </td>
                      <td className="px-4 py-3 font-black">R$ {Number(order.total).toFixed(2).replace(".", ",")}</td>
                      <td className="px-4 py-3 text-sm text-black/55">{new Date(order.criado_em).toLocaleTimeString("pt-BR")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
