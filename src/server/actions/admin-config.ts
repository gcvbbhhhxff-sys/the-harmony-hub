...
async function uploadRestaurantAsset(db: Awaited<ReturnType<typeof createClient>>, file: File, folder: "logo" | "background") {
  const validationError = await validateImage(file);
  if (validationError) return { ok: false, message: validationError } as const;
  ...
  if (upload.error) {
    ...
    return { ok: false, message: `Erro ao fazer upload ${folder === "logo" ? "da logo" : "da imagem de fundo"}.` } as const;
  }
  ...
  if (updateError) {
    ...
    return { ok: false, message: `Upload concluído, mas não foi possível salvar ${folder === "logo" ? "a logo" : "a imagem de fundo"} no restaurante.` } as const;
  }
  revalidatePath("/");
  revalidatePath("/admin/configuracoes");
