...
import { deleteAddon, deleteCategory, deleteOption, deleteOptionGroup, deleteProduct, linkAddon, saveAddon, saveCategory, saveOption, saveOptionGroup, saveProduct, toggleProduct, uploadProductImage } from "@/server/actions/admin-menu";
...
export default function MenuAdminClient({ initial }: Props) {
  ...
  const run = (fn: () => Promise<{ ok: boolean; message?: string }>, shouldRefresh = false) => startTransition(async () => {
    try {
      const result = await fn();
      setMsg(result.ok ? "Alterações salvas." : result.message || "Não foi possível concluir.");
      if (result.ok && shouldRefresh) refresh();
    } catch (error) {
    ...
    <main className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card className="rounded-2xl border-black/8 p-5">
          ...
          <div className="mt-4 grid gap-2">
            ...
            <Button disabled={pending || !productForm.category_id} onClick={() => run(() => saveProduct({ ...productForm, imagem_url: productForm.imagem_url || null }), true)}>{productForm.id ? "Salvar alterações" : "Criar produto"}</Button>
          </div>
        </Card>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card className="rounded-2xl border-black/8 p-5">
          ...
          <div className="mt-4 grid gap-2"><select className="h-10 rounded-md border border-black/10 bg-white px-3" value={optionForm.group_id} onChange={(e) => setOptionForm({ ...optionForm, group_id: e.target.value })}>{data.groups.map((group) => <option key={group.id} value={group.id}>{group.nome}</option>)}</select><Input placeholder="Nome" value={optionForm.nome} onChange={(e) => setOptionForm({ ...optionForm, nome: e.target.value })} /><Input type="number" min="0" step="0.01" placeholder="Acréscimo" value={optionForm.preco_extra} onChange={(e) => setOptionForm({ ...optionForm, preco_extra: Number(e.target.value) })} /><Input type="number" min="0" placeholder="Ordem" value={optionForm.ordem} onChange={(e) => setOptionForm({ ...optionForm, ordem: Number(e.target.value) })} /><label className="flex items-center gap-2 text-sm"><Checkbox checked={optionForm.ativo} onChange={(e) => setOptionForm({ ...optionForm, ativo: e.target.checked })} /> Ativa</label><Button disabled={pending || !optionForm.group_id} onClick={() => run(() => saveOption(optionForm), true)}>{optionForm.id ? "Salvar opção" : "Criar opção"}</Button></div>
      ...
      <Card className="rounded-2xl border-black/8 p-5">
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-3"><Input placeholder="Nome" value={addonForm.nome} onChange={(e) => setAddonForm({ ...addonForm, nome: e.target.value })} /><Input type="number" min="0" step="0.01" value={addonForm.preco} onChange={(e) => setAddonForm({ ...addonForm, preco: Number(e.target.value) })} /><Button disabled={pending} onClick={() => run(() => saveAddon(addonForm), true)}>{addonForm.id ? "Salvar adicional" : "Criar adicional"}</Button></div>
