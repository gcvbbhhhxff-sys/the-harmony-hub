...
export function LoginForm(){
  ...
  return <main className="min-h-screen grid place-items-center bg-[radial-gradient(circle_at_top,rgba(211,163,40,.18),transparent_34rem)] bg-[var(--color-secondary)] p-4">
    <Card className="w-full max-w-md rounded-3xl p-7 shadow-[0_24px_70px_rgba(0,0,0,.3)]">
      ...
      <p className="mt-2 text-sm text-black/55">Use seu usuário administrativo para acessar o painel.</p>
      <div className="mt-6 grid gap-3">
        <Input type="text" autoComplete="username" autoCapitalize="none" spellCheck={false} placeholder="Usuário ou e-mail" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")submit()}}/>
