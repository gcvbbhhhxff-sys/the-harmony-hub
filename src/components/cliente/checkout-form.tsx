...
import { sendPhoneOtp, verifyPhoneOtp, startAnonymousSession } from "@/server/actions/auth";
...
export function CheckoutForm({ initialAddresses, authenticated, initialPhone }: CheckoutProps) {
  ...
  const continueAsGuest = () => startTransition(async () => {
  ...
  const submit = () =>
    startTransition(async () => {
      if (!identified) {
        setMessage("Entre como visitante ou confirme seu telefone antes de continuar.");
        return;
      }
  ...
  if (!hydrated) {
    return (
      <main className="mx-auto flex min-h-[60vh] max-w-3xl items-center justify-center px-4 py-10 md:px-6">
        <Card className="w-full p-8 text-center shadow-[0_20px_60px_rgba(17,17,17,.08)]">
